...
test("Axios client response handler", () => {
  ...
  const mockResponseGenerator = function (
    data?: undefined | number | boolean | string | { [key: string]: any }
  ) {
    return {
      data: data,
      status: 42,
  ...
  expect(error.toString()).toBe(
    "DrupalError: 107 Received an error with invalid JSON from server: Invalid JSON"
  );

  error = api.getDrupalError(mockResponseGenerator({ responseText: 1337 }));
  expect(error.toString()).toBe(
    "DrupalError: 107 Received an error with invalid JSON from server: 1337"
  );
});
...
test("Axios client error - 500 response", async () => {
  const api = new AxiosClient(createStubClient());
  expect.assertions(1);
  await api.call("GET", "/mock-err-500", {}).catch((err) => {
...
test("Axios client error - proper json response", async () => {
  const api = new AxiosClient(createStubClient());
  expect.assertions(1);
  await api.call("GET", "/mock-err-proper-response", {}).catch((err) => {
    expect(err).toBeInstanceOf(DrupalError);
  });
});
